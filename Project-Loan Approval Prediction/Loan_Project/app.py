...
def load_model_and_compute_metrics():
    ...
    if os.path.exists(DATA_PATH):
        app.logger.info("Dataset found. Computing metrics...")
        ...
        metrics_cache = {
            ...
            "confusion_matrix": cm,
            "sample_count": len(df),
            "support_vectors": len(model.named_steps['classifier'].support_) if hasattr(model,
                                                                                        'named_steps') and hasattr(
                model.named_steps.get('classifier', None), 'support_') else 1248
        }
        app.logger.info("Metrics calculated successfully!")
    else:
        app.logger.warning(f"Data file not found at: {DATA_PATH}")
...
def home():
    ...
    if model is None:
        app.logger.error("Attempted to access Home page, but model is missing")
...
@app.route('/dashboard')
def dashboard():
    app.logger.info("Accessing Dashboard page")
...
def predict():
    ...
    try:
        ...
        app.logger.info(f"Prediction requested with payload: {data}")
        ...
        input_data = pd.DataFrame([{
            ...
            'previous_loan_defaults_on_file_Y': has_default
        }])

        prediction = int(model.predict(input_data)[0])
        status = "Approved" if prediction == 1 else "Rejected"

        app.logger.info(f"Prediction completed successfully. Result: {status} (code: {prediction})")
        ...
        app.logger.error(f"Error occurred during prediction processing: {str(e)}")
